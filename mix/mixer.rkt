...
(define mix
  '((read program division vs0)
    ...
    (pending-loop               (:= pp (car (set-first pending)))
    ...
    (command-if-dynamic         (:= pp-then (caddr command))
                                ...
                                (:= pending (set-union pending (set-subtract (set (list pp-then vs) (list pp-else vs)) marked)))
                                (:= code (extend-block code (list 'if (reduce-expr expr vs division) (name-block pp-then vs) (name-block pp-else vs))))
                                (goto bb-loop-cond))
